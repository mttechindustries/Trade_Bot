import { TickerData, CandlestickData, OrderBookData } from '../types';
...
class RealTimeMarketDataService {
  ...
  private exchanges: Record<string, ExchangeConfig> = {
    binance: {
      name: 'Binance',
      restApiBase: 'https://api.binance.com/api/v3',
      websocketBase: 'wss://stream.binance.com:9443/ws',
      rateLimitPerSecond: 10
    },
    coinbase: {
      name: 'Coinbase Pro',
      restApiBase: 'https://api.exchange.coinbase.com',
  ...
  /**
   * Get ticker data from multiple exchanges with caching
   */
  async getTickerData(symbols: string[]): Promise<TickerData[]> {
    return this.getRealTimeTickerData(symbols);
  }
